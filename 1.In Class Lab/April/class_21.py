...
def do_fourier(TERMS, wave):
    ...
    '''
    compute_a0(func, period=2*np.pi, num_points=1000) | return (1 / period) * result
    compute_an(func, n, period=2*np.pi, num_points=1000) | return (2/period) * result
    compute_bn(func, n, period=2*np.pi, num_points=1000) | return (2/period) * result
    compute_coefficients(func, n_terms, period=2*np.pi, num_points=1000) | return a0, an, bn
    fourier_series_approximation(x, a0, an, bn, period=2*np.pi) | return result
    compute_partial_approximations(x, a0, an, bn, period=2*np.pi) | return approximations
    '''

    # Compute Fourier series coefficients

    a0, an, bn = fs.compute_coefficients(wave, TERMS,period=2*np.pi, num_points=1000)
    y_approx = fs.fourier_series_approximation(x, a0, an, bn, period=2*np.pi)
    # Calculate the Fourier approximation
        
    # Calculate partial approximations
    partial_approx = fs.compute_partial_approximations(x, a0, an, bn, period=2*np.pi)
    ...
    plt.yscale('log')
    plt.savefig('coefficient_spectrum.png')
    plt.close()
    ...
    plt.savefig('convergence_rate_linear.png')
    plt.close()
    ...
    plt.xlabel('Number of Terms')
    plt.ylabel('RMS Error')
    plt.savefig('convergence_rate_log.png')
    plt.close()
...
def my_signal(x):
    return triangle_wave(np.sin(x**2))
